...
{
    LCD_DC(0);//命令模式
    spi2_write_data(&cmd,1);//发送一个字节命令
}
...
{
    LCD_DC(1);//数据模式
    spi2_write_data(&data,1);//发送一个字节数据
}

/*发送16位数据（颜色）函数*/
void lcd_write_data16(uint16_t data)
{
    uint8_t databuf[2] = {0,0};//定义一个2字节数组，用于存储16位数据
    databuf[0] = data >> 8 ;//高字节
    databuf[1] = data & 0xFF ;//低字节
    LCD_DC(1);//数据模式
    spi2_write_data(databuf,2);//发送2字节数据
}
...
{
    LCD_DC(1);//数据模式
    spi2_write_data(data,length);//发送n字节数据
}
...
void lcd_init(void)
{
    ...
    // spidevice_structure.spics_io_num = 1ull << LCD_CS_PIN;
    ...
    gpio_init_struct.pin_bit_mask = 1ull << LCD_DC_PIN ;         
    ...
    gpio_init_struct.intr_type = GPIO_INTR_DISABLE;                
    gpio_init_struct.mode = GPIO_MODE_OUTPUT;                      
    gpio_init_struct.pin_bit_mask = 1ull << LCD_BLK_PIN;           
    ...
    gpio_init_struct.intr_type = GPIO_INTR_DISABLE;                
    gpio_init_struct.mode = GPIO_MODE_OUTPUT;                   
    gpio_init_struct.pin_bit_mask = 1ull << LCD_RST_PIN;        
    gpio_init_struct.pull_down_en = GPIO_PULLDOWN_DISABLE;       
    gpio_init_struct.pull_up_en = GPIO_PULLUP_ENABLE;               
    gpio_config(&gpio_init_struct);                              
